...
subjdirs=`cat /scratch/groups/Projects/P1300/fMRI_data/elapsed_time/gradient/subject_level/subjectlist_offtask.txt`
...
for n in $subjdirs; do
	...
	fslcc -m $Mask --noabs -t -1 $n $Grad1 >> gradient1_offtask.txt
	fslcc -m $Mask --noabs -t -1 $n $Grad2 >> gradient2_offtask.txt
	fslcc -m $Mask --noabs -t -1 $n $Grad3 >> gradient3_offtask.txt
done
