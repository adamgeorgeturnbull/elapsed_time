...
for f in `ls /scratch/groups/Projects/P1300/fMRI_data/elapsed_time/gradient/leave_one_out/*es*.txt | sort -V`; do
	subjlists=`cat $f`
	echo $subjlists >> log_offtask.txt
	fslmerge -t merge_tmp.nii.gz $subjlists
	fslmaths merge_tmp.nii.gz -Tmean mean_tmp.nii.gz
	fslcc -m $Mask --noabs -t -1 -p 3 mean_tmp.nii.gz $Grad1 >> gradient1_es.txt
	fslcc -m $Mask --noabs -t -1 -p 3 mean_tmp.nii.gz $Grad2 >> gradient2_es.txt
	fslcc -m $Mask --noabs -t -1 -p 3 mean_tmp.nii.gz $Grad3 >> gradient3_es.txt
	rm *tmp*
done
