...
subjdirs=`cat /scratch/groups/Projects/P1300/fMRI_data/elapsed_time/gradient/subject_level/subjectlist_elapsed_time_TT.txt`
...
for n in $subjdirs; do
	...
	cat tmp.txt >> betas_elapsed_timeTT1.txt
	fsl_glm -i $n -d $Grad2 -o tmp.txt --demean -m $Mask
	cat tmp.txt >> betas_elapsed_timeTT2.txt
	fsl_glm -i $n -d $Grad3 -o tmp.txt --demean -m $Mask
	cat tmp.txt >> betas_elapsed_timeTT3.txt
done
